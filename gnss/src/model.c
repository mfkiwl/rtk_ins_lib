...
RTK_RAM_CODE void blh2xyz(const double *blh, double *xyz)
{
    // lat, lon, ht => ecef xyz
    double a = RE_WGS84, finv = 1.0 / FE_WGS84;
...
RTK_RAM_CODE void xyz2blh(const double *xyz, double *blh)
{
    // ecef xyz => blh
    double a = RE_WGS84, finv = 1.0 / FE_WGS84;
    double f = 1.0 / finv, e2 = 2 * f - f * f;
...
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern double time2doy(gtime_t t)
{
    double ep[6];
...
RTK_RAM_CODE extern void ned2xyz(double C_en[3][3], double *ned, double *covNED, double *xyz, double *covXYZ)
{
    double temp[3][3] = { 0 };
    int i, j, k;
    xyz[0] = C_en[0][0] * ned[0] + C_en[0][1] * ned[1] + C_en[0][2] * ned[2];
    xyz[1] = C_en[1][0] * ned[0] + C_en[1][1] * ned[1] + C_en[1][2] * ned[2];
    xyz[2] = C_en[2][0] * ned[0] + C_en[2][1] * ned[1] + C_en[2][2] * ned[2];
    if (covXYZ != NULL && covNED != NULL)
    {
        /* covXYZ = C_en*covNED*C_en' */
        for (i = 0; i < 3; ++i)
        {
            for (j = 0; j < 3; ++j)
            {
                ...
                for (k = 0; k < 3; ++k)
                {
                    temp[i][j] += C_en[i][k] * covXYZ[SMI(k, j)];
                }
            }
        }
        ...
        for (i = 0; i < 3; ++i)
        {
            for (j = 0; j < 3; ++j)
            {
                ...
                for (k = 0; k < 3; ++k)
                {
                    covNED[SMI(i, j)] += temp[i][k] * C_en[j][k];
                }
            }
        }
    }
...
RTK_RAM_CODE extern void xyz2ned(double C_en[3][3], double *xyz, double *covXYZ, double *ned, double *covNED)
{
	double temp[3][3] = {0};
	int i, j, k;
...
RTK_RAM_CODE extern void ned2_Clateral_lon(const double heading, double C_en[3][3])
{
    C_en[0][0] = cos(heading);
    C_en[1][0] = -sin(heading);
    C_en[2][0] = 0.0;
    C_en[0][1] = sin(heading);
    C_en[1][1] = cos(heading);
    C_en[2][1] = 0.0;
    C_en[0][2] = 0.0;
    C_en[1][2] = 0.0;
    C_en[2][2] = 1.0;
...
RTK_RAM_CODE extern void ned2lld(double C_en[3][3], double *ned, double *covNED, double *lld, double *covLLD)
{
    double temp[3][3] = { 0 };
    int i, j, k;
    lld[0] = C_en[0][0] * ned[0] + C_en[0][1] * ned[1] + C_en[0][2] * ned[2];
    lld[1] = C_en[1][0] * ned[0] + C_en[1][1] * ned[1] + C_en[1][2] * ned[2];
    lld[2] = C_en[2][0] * ned[0] + C_en[2][1] * ned[1] + C_en[2][2] * ned[2];
    if (covLLD != NULL && covNED != NULL)
    {
        /* covXYZ = C_en*covNED*C_en' */
        for (i = 0; i < 3; ++i)
        {
            for (j = 0; j < 3; ++j)
            {
                ...
                for (k = 0; k < 3; ++k)
                {
                    temp[i][j] += C_en[i][k] * covNED[SMI(k, j)];
                }
            }
        }
        ...
        for (i = 0; i < 3; ++i)
        {
            for (j = 0; j < 3; ++j)
            {
                ...
                for (k = 0; k < 3; ++k)
                {
                    covLLD[SMI(i, j)] += temp[i][k] * C_en[j][k];
                }
            }
        }
    }
...
RTK_RAM_CODE extern void blhdiff(double *blh, double *blh_ref, double *ned)
{
	double C_en[3][3] = {0};
	double xyz[3] = {0};
	double xyz_ref[3] = {0};
...
RTK_RAM_CODE extern double satazel(const double *pos, const double *e, double *azel)
{
...
RTK_RAM_CODE extern double geodist(const double *rs, const double *rr, double *e)
{
	double r;
	int i;
...
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern double geovel(const double *rs, const double *rr, double *e)
{
	double rate = 0.0;
	int i;
	double vs[3] = {0.0};
...
RTK_RAM_CODE extern double tropmodel(const double *blh, const double *azel, double humi)
{
...
RTK_RAM_CODE double interpc(const double coef[], double lat)
{
    int i = (int)(lat / 15.0);
...
RTK_RAM_CODE double mapf(double el, double a, double b, double c)
{
    double sinel = sin(el);
    return (1.0 + a / (1.0 + b / (1.0 + c))) / (sinel + (a / (sinel + b / (sinel + c))));
}

RTK_RAM_CODE double nmf(gtime_t time, const double pos[], const double azel[],double *mapfw)
{
    /* ref [5] table 3 */
...
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE double tropmapf(gtime_t time, const double pos[], const double azel[],
    double *mapfw)
{
#ifdef IERS_MODEL
...
* return : none
*-----------------------------------------------------------------------------*/
RTK_RAM_CODE extern void deg2dms(double deg, double *dms, int ndec)
{
...
RTK_RAM_CODE extern int print_nmea_gga(double *ep, double *xyz, int nsat, int type, 
	double dop, double age, char *buff)
{
...
RTK_RAM_CODE extern void RealToArray(double real, char *a, unsigned char id, unsigned char dd) 
{	// id:integer digits，dd:decimal digits
	...
	if (id == 0)
	{
		...
		else if (abs(real) < 100.0)
		{
			id = 2;
		}
		else if (abs(real) < 1000.0)
		{
			id = 3;
		}
		else if (abs(real) < 10000.0)
		{
			id = 4;
		}
		else
	...
	if (real >= 0)
	{
		temp1 = real * pow(10.0, dd);
		temp2 = ((int)(real)) * pow(10.0, dd);
		temp2 = temp1 - temp2; 
		temp1 = (int)real;	 
		for (i = id; i > 0; i--)
		{
...
RTK_RAM_CODE extern char *itoa_user(int num, char *str, int radix)
{ 
